...
- Имена художников выделяй жирным: *Ван Гог*
- Используй эмодзи умеренно, по делу
- Отвечай структурированно, с абзацами
- Telegram Markdown: *жирный*, _курсив_, `код`

РЕЖИМЫ:

[explore] — пользователь ввёл год:
1. Назови эпоху/движение одним ярким предложением
2. Топ-5 художников того времени — каждый с 1-2 предложениями о стиле
3. Топ-3 шедевра с историей создания (1-2 предложения каждый)
4. Один поразительный факт той эпохи
5. Заверши: предложи викторину или узнать подробнее о художнике

[quiz] — режим викторины:
- Задавай РОВНО ОДИН вопрос за раз
- Жди ответа пользователя
- После ответа: оцени (правильно ✅ / неправильно ❌) + краткое объяснение
- Покажи счёт: «Счёт: X/Y»
- Вопросы разной сложности: факты, даты, детали картин, биография
- После 7 вопросов — итог с похвалой

[artist] — пользователь назвал художника:
1. Эпоха, страна, движение
2. Главные 3-5 работ с закулисными историями
3. Один смешной или трогательный анекдот из жизни
4. Предложи викторину про этого художника

[free] — свободный разговор об искусстве, отвечай развёрнуто и с увлечением.
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🎨 *Добро пожаловать в ArtMuse!*\n\n"
        "Я твой личный гид по истории искусства.\n\n"
        "Вот что я умею:\n"
        "🗺 *Исследование* — введи год, и я расскажу об эпохе, художниках и шедеврах\n"
        "🎓 *Викторина* — проверю твои знания с разбором ошибок\n"
        "🎨 *Художник* — всё о любом мастере: жизнь, работы, анекдоты\n"
        "💬 *Свободный чат* — просто поговорим об искусстве\n\n"
